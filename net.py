...
def on_off( hosts,test_time,alpha1,alpha2,lock ):
    ...
    while b <  a :
        b = time.time()
        t1 = random.paretovariate( alpha1 ) * 0.010
        t2 = random.paretovariate( alpha2 ) * 0.010
        if (t1+t2)>(a-b+3):
            continue
        if t1 < 0.011:
            continue
...
def poisson( hosts,test_time,lambd1,lambd2,lock ):
    ...
    while b <  a :
        b = time.time()
        t1 = random.expovariate( lambd1 )
        t2 = random.expovariate( lambd2 )
        if (t1+t2)>(a-b+3):
            continue
        if t1 < 0.011:
            continue
...
#        client.cmd('iperf -t'+' '+test_time+' -c'+' '+server.IP()+' &')
    ...
    def poisson_multi( self,test_time = 15,lambd1 = 13.3 ,lambd2 = 17 ):
    ...
    def host_ports_config( self,bandwidth1 = '0.01',bandwidth2 = '0.05' ):        
    ...
    def self_define( self,arg1 = '10' ):
        ...
        for i in range(int(arg1)):
            host = host_list[i]
            host.cmd( 'vlc http://10.0.1.1:8080/test &' )
