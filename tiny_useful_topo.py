...
def myNetwork():
    ...
    for i in xrange(1,101):
        ...
        h.append( hs )
    #Adding the special host "h121"
    hs = net.addHost('h101',cls = Host,ip = '10.0.1.1',defaultRoute=None)
    h.append( hs )

    info( '*** Adding links ***\n')
    #Adding the special link between h121 and s101
    net.addLink(core_sw[0],h[100])
    ...
    for i in range(dist_num):
        for j in range(fanout_num):
            net.addLink(dist_sw[i],aces_sw[i*fanout_num+ j])
    ...
    for i in xrange(1,aces_num):
        for j in range(10):
            net.addLink(aces_sw[i], h[(i-1)*10+j+50])
